...
> **2026-06-29 landing-page rebuilt (v7 → cork-board "v3") — supersedes the "Layout decisions (v7)", "What's working", and "Open issues" sections below.** The landing page (`index.html` + `styles/global.css`) was replaced wholesale with the Claude Design v3 handoff (bundle in `design/claude-design-import-v3/`; high-fidelity HTML/CSS reference + the club's real photos). This was the user's explicit, locked choice ("that's how I want it to look entirely"). New structure: **cork bulletin-board** as the whole-page background (not the old bulletin-board *wall image* + tiled-paper approach), a single **torn cream flyer** taped to it whose ragged edges come from an **SVG `feDisplacementMap` filter** (`#paper-rip`) — this **resolves the long-standing side-torn-edge geometry bug** (no more rotated-strip hack; the three paths A/B/C in Open issues are now moot). Hero = ram skull + Anton "SKATE CLUB / at UNC" wordmark with the "at UNC" line on a Carolina swatch. Griptape **nav band** (Spots → `spots/index.html`, Meetings, About → `#the-wall`, GroupMe, IG — real hrefs) with an **animated SVG skater** grinding a chrome rail (Web Animations API, now `prefers-reduced-motion`-guarded). Info grid (meetings + "for anyone" inclusivity list) with the **Pluto YouTube embed** (`youtube-nocookie/FUS0ikotKhY`) in a taped Polaroid. Footer strip + **"// the wall" photo gallery** of pushpinned real club photos. The prototype's `image-slot.js` drag-drop element was **dropped** — replaced with real `<img>` tags; two slots (crew strip + "fully sent") are intentionally empty `.slot-empty` placeholders awaiting real photos. Brand tokens/typefaces are unchanged from the locked palette. Verified rendering in-browser (no console errors); security pass run before push. The **palette and typeface locks below still hold**; only the *layout decisions* are superseded.
