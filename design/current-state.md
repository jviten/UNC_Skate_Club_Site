...
**Backend foundation — built (not yet run).** `backend/` holds FastAPI + SQLAlchemy + Alembic, PostGIS models (`spots` + the v2-anticipating `videos` table), read-only `/api/spots` (bbox) + `/api/spots/{id}`, an editable `seed_data.json` + idempotent seed script (4 placeholder Chapel Hill/Carrboro spots), and `snapshot.py`. `security` pass: no blockers; fixed CORS prod-warning + bbox range guard. **Pending**: stand up env (`uv sync` → also generates the missing `uv.lock`, `docker compose up`, migrate/seed/snapshot/health) — needs `uv` installed; runtime never verified yet. Local dev server for the frontend currently runs via `python -m http.server 5500` at the repo root.
